...
class VanillaBlock(nn.Module):
    ...
    def __init__(self, dim, num_heads, mlp_ratio=4., mlp_out_ratio=1., qkv_bias=True, qk_scale=None, drop=0.,
        ...
        self.attn_s = VanillaAttention(dim, num_heads=num_heads, qkv_bias=qkv_bias, qk_scale=qk_scale, attn_drop=attn_drop, proj_drop=drop)
...
class Block(nn.Module):
    ...
    def __init__(self, dim, num_heads, mlp_ratio=4., mlp_out_ratio=1., qkv_bias=True, qk_scale=None, drop=0.,
        ...
        if 'stage_stg' == st_mode or 'stage_tgs' == st_mode:
            self.attn_g = Attention(dim, num_heads=num_heads, qkv_bias=qkv_bias, qk_scale=qk_scale, attn_drop=attn_drop, proj_drop=drop, st_mode="temporal")
            self.joint_emb = nn.Linear(21 * dim, dim)
            self.mlp_g = MLP(in_features=dim, hidden_features=mlp_hidden_dim, out_features=mlp_out_dim,act_layer=act_layer, drop=drop)
            self.norm1_g = norm_layer(dim)
            self.norm2_g = norm_layer(dim)
    ...
    def forward(self, x, seqlen=1):
        if self.st_mode == 'stage_st':
            ...
            x = x + self.drop_path(self.attn_s(self.norm1_s(x), seqlen))
            x = x + self.drop_path(self.mlp_s(self.norm2_s(x)))
        elif self.st_mode == 'stage_stg':
            x = x + self.drop_path(self.attn_s(self.norm1_s(x), seqlen))
            x = x + self.drop_path(self.mlp_s(self.norm2_s(x)))
            x = x + self.drop_path(self.attn_t(self.norm1_t(x), seqlen))
            x = x + self.drop_path(self.mlp_t(self.norm2_t(x)))
            x = x + self.drop_path(self.attn_g(self.norm1_g(self.global_attn(x)), seqlen))
            x = x + self.drop_path(self.mlp_g(self.norm2_g(x)))
        elif self.st_mode == 'stage_tgs':
            x = x + self.drop_path(self.attn_t(self.norm1_t(x), seqlen))
            x = x + self.drop_path(self.mlp_t(self.norm2_t(x)))
            x = x + self.drop_path(self.attn_g(self.norm1_g(self.global_attn(x)), seqlen))
            x = x + self.drop_path(self.mlp_g(self.norm2_g(x)))
    ...
    def temporal_conv(self, x, F=27):
        B, N, C = x.shape
        x = rearrange(x, '(b f) n c -> (b n) c f', f=F)
        x = self.conv_t(x)
        x = rearrange(x, '(b n) c f -> (b f) n c ', n=N)
        return x

    def global_attn(self, x):
        B, N, C = x.shape
        x = rearrange(x, 'b n c -> b (n c)')
        x = self.joint_emb(x).reshape(B, 1, C)
        return x
...
if __name__ == '__main__':
    x = torch.rand([16, 27, 21, 3]).cuda()
    model = DSTFormer(dim_in=3, dim_feat=256, depth=2, num_heads=4, mlp_ratio=2, num_joints=21, maxlen=27).cuda()
    out = model(x)
    print(out.size())
