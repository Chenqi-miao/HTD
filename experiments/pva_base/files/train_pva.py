"""
train_pva.py — PVANet 训练入口
=================================
用法:
  uv run --no-sync python3 train_pva.py --exp pva_base
  uv run --no-sync python3 train_pva.py --exp pva_v1
  uv run --no-sync python3 train_pva.py --exp pva_v2

实验目录结构:
  experiments/{exp_name}/
    config.py        ← 参数文件
    log/             ← TensorBoard + 文本日志
    checkpoint/      ← 模型保存
    report.md        ← 实验报告
"""
...
def evaluate(model, test_loader):
    """跑验证集，返回 (init_mpjpe, refine_mpjpe)"""
    model.eval()
...
def main():
    ...
    parser.add_argument('--exp', required=True, help='experiment name (e.g. pva_base)')
    parser.add_argument('--resume', default='', help='checkpoint path to resume from')
    args = parser.parse_args()

    cfg, exp_name = load_config(args.exp)
    exp_dir = cfg.exp_dir
    ...
    # ── 模型 ──
    from model.PVANet import PVANetModel
    model = PVANetModel(
        num_frame=cfg.seq_len,
        num_joints=cfg.joint_num,
        num_view=cfg.view_num,
    ...
    shutil.copytree('./model', f'{code_dir}/model/', dirs_exist_ok=True)
    shutil.copytree('./dataset', f'{code_dir}/dataset/', dirs_exist_ok=True)
    shutil.copytree('./utils', f'{code_dir}/utils/', dirs_exist_ok=True)
    shutil.copy2(__file__, f'{code_dir}/train_pva.py')
    ...
    for epoch in range(start_epoch, cfg.total_epoch):
        ...
        if epoch % cfg.eval_interval == 0:
            init_mpjpe, refine_mpjpe = evaluate(model, test_loader)
            logger.info(f'         Init MPJPE={init_mpjpe:.4f} mm | Refine MPJPE={refine_mpjpe:.4f} mm')
