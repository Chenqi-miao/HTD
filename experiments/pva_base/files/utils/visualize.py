...
def draw_2d_skeleton(image, pose_uv, color=None):
    """
    ...
    for joint_ind in range(pose_uv.shape[0]):
        if color is None:
            joint_color = color_hand_joints[joint_ind] * np.array(255)
        else:
            joint_color = color * np.array(255)
        joint = pose_uv[joint_ind, 0].astype('int32'), pose_uv[joint_ind, 1].astype('int32')
        cv2.circle(
            skeleton_overlay, joint,
            radius=marker_sz, color=joint_color, thickness=-1,
        ...
        if joint_ind == 0:
            ...
            cv2.line(
                ...
                color=joint_color, thickness=int(line_wd),
                lineType=cv2.CV_AA if cv2.__version__.startswith('2') else cv2.LINE_AA)
        else:
...
def render_mesh(img, mesh, cam_param):
    ...
    #                                               baseColorFactor=(color[0], color[1], color[2], 0)) # (B, G, R, ?)
