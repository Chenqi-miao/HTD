...
def rot_img(img_list, label2d_list, theta=0, img_size=256):
    img_list_out = []
    center = np.array([img_size / 2, img_size / 2, 1], dtype='float32')
    M = get_rotation_mat(center, theta)

    for img in img_list:
        img_list_out.append(cv.warpAffine(img, M[0:2, ...], dsize=(img_size, img_size)))
    ...
    return img_list_out, label2d_list_out, M


def video_cut_img(img_list, label2d_list, camera=None, radio=0.7, img_size=256):
    Min = []
    Max = []
    for label2d in label2d_list:
        Min.append(np.min(label2d, axis=0))
    ...
    return img_list_out, label2d_list_out, camera, scale
